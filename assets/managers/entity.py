...
class Entity():
    ...
    def __init__(self, AItype="simple", texturepath="player", xp=0):
        ...
        self.ai_type = AItype
        self.index = 0
        self.portraits = {"default": pygame.transform.scale(pygame.image.load(os.path.join(
            constants.PORTRAIT_PATH, "default.png")), (32*constants.screen_scale, 32*constants.screen_scale))}
        self.x = 0
        self.y = 0
        self.x_vel = 0
        self.y_vel = 0
        ...
        self.texture_size = [
            constants.HALF_BLOCK_SIZE, constants.HALF_BLOCK_SIZE]
        self.grounded = True
        ...
        self.overlay = pygame.transform.scale(
    ...
    def update_physics(self, up=False, left=False, right=False):
        ...
        angle = math.degrees(math.atan(
            (pygame.mouse.get_pos()[1]-pos[1])/(int(pygame.mouse.get_pos()[0]-pos[0])+0.1)))
...
class DynamicTransitionObject():
    ...
    def check(self):
        if self.rect.collidepoint((common.player.x, common.player.y)):
            if self.id == 0:
                try:
                    common.run.intermediary.levelarray[str(
            ...
            if len(common.level_transitions) == 0:
                raise ValueError(
                    "Destination level "+common.loaded_level.name+" has no valid entrance")
