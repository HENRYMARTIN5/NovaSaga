...
class Box():
    ...
    def Draw(self, forcedraw=False):
        if not self.hidden or forcedraw:  # check wheter or not the box is hidden, or if its being forced to draw
            # unflip player y and the rect y positions
            ...
            if self.rect.colliderect(e):  # check if the box is onscreen
                ...
                if not (y_value1 == y_value2 or x_value_1 == x_value_2):
                    ...
                    if (y_value1 == camera_y and y_value2 == camera_height+camera_y) or (y_value1 == camera_height+camera_y and y_value2 == camera_y):
                        if player_x >= x:  # statements with 2 points being made account for the rare scenario in which points are on opposite edges
                            ...
                            points.append(
                                (camera_width+camera_x, camera_height+camera_y))
                        ...
                        if player_y >= y:  # account for points on adjacent edges
                            ...
                            points.append(
                                (camera_width+camera_x, camera_height+camera_y))
                    # accounts for the points being on adjacent edges
                    elif y_value2 == camera_y or y_value2 == camera_height+camera_y:
...
class Level():
    ...
    def load(self, levelname="test"):
        ...
        # load from collision file if display file doesnt exist
        ...
        try:
            for i in data["level_transitions"]:
                if i["style"] == "old":
                    common.level_transitions.append(entity.TransitionObject(pygame.Rect(
                        i["x"], i["y"], i["w"], i["h"]), (i["dest_x"], i["dest_y"]), i["dest_level"]))  # add level transitions
                elif i["style"] == "new":
                    common.level_transitions.append(entity.DynamicTransitionObject(pygame.Rect(
        ...
        for i in range(0, self.collision_texture.get_width()-1):  # for each x
            for j in range(0, self.collision_texture.get_height()-1):  # for each y
                ...
                # check the point isnt already in a box
                if rect.collidelist(boxrectlist) == -1 and self.collision.get_at((i, j)) == 1:
                    ...
                    # go as far down as possible, keeping the shortest column for the height
                    for l in range(newbox.x, newbox.x+newbox.w):
                        m = 0
                        while True:
...
class Map():
    def GrowingTree(surface, extraconnections=1000):  # maze generation algorithm
        ...
        for i in range(0, extraconnections):
            pos = (random.randint(3, int(surface.get_width()/2))*2-3,
                   random.randint(3, int(surface.get_height()/2))*2-3)
            value = random.randint(0, 3)
            if value == 0:
                pygame.draw.line(
                    surface, constants.PATH_TILE_COLOR, pos, (pos[0], pos[1]-2))
            elif value == 1:
                pygame.draw.line(
                    surface, constants.PATH_TILE_COLOR, pos, (pos[0], pos[1]+2))
            elif value == 2:
    ...
    def __init__(self, seed, type=0):
        ...
        if self.type == 0:  # intermediary type
            ...
            self.doorways.append(
                [2*random.randint(5, 15)-1, 2*random.randint(5, 15)-1])  # memory tile pos
            self.doorways.append(
                [2*random.randint(20, 30)-1, 2*random.randint(5, 15)-1])  # dream tile pos
            # labyrinth tile pos
            self.doorways.append(
                [2*random.randint(35, 45)-1, 2*random.randint(5, 15)-1])
            # fracture tile pos
            self.doorways.append(
                [2*random.randint(35, 45)-1, 2*random.randint(20, 30)-1])
            # abstract tile pos
            self.doorways.append(
                [2*random.randint(35, 45)-1, 2*random.randint(35, 45)-1])
            self.doorways.append(
                [2*random.randint(20, 30)-1, 2*random.randint(35, 45)-1])  # abyss tile pos
            # trauma tile pos, not what it seems
            self.doorways.append(
                [2*random.randint(5, 15)-1, 2*random.randint(35, 45)-1])
            # awakening tile pos
            self.doorways.append(
                [2*random.randint(5, 15)-1, 2*random.randint(20, 30)-1])
            # auric door tile pos, also the end of the game
            self.doorways.append(
                [2*random.randint(30, 35)-1, 2*random.randint(20, 30)-1])
            ...
            for i in self.secretrooms:  # make sure secret rooms only have 1 tile connection
                ...
                self.map.set_at((i.pos[0]-1, i.pos[1]),
                                constants.MAP_BACKGROUND_COLOR)
                self.map.set_at((i.pos[0], i.pos[1]-1),
                                constants.MAP_BACKGROUND_COLOR)
                self.map.set_at((i.pos[0], i.pos[1]+1),
                                constants.MAP_BACKGROUND_COLOR)
                self.map.set_at((i.pos[0]+1, i.pos[1]),
            ...
            # memory tile
            self.map.set_at(self.doorways[1].pos, constants.MEMORY_TILE_COLOR)
            self.map.set_at(
                self.doorways[2].pos, constants.DREAM_TILE_COLOR)  # dream tile
            # labyrinth tile
            self.map.set_at(self.doorways[3].pos,
                            constants.LABYRINTH_TILE_COLOR)
            # fracture tile
            self.map.set_at(self.doorways[4].pos,
                            constants.FRACTURE_TILE_COLOR)
            # abstract tile
            self.map.set_at(self.doorways[5].pos,
                            constants.ABSTRACT_TILE_COLOR)
            self.map.set_at(
                self.doorways[6].pos, constants.ABYSS_TILE_COLOR)  # abyss tile
            # trauma tile
            self.map.set_at(self.doorways[7].pos, constants.TRAUMA_TILE_COLOR)
            # awakening tile
            self.map.set_at(self.doorways[8].pos,
                            constants.AWAKENING_TILE_COLOR)
            # auric door tile
            self.map.set_at(self.doorways[9].pos,
                            constants.AURIC_DOOR_TILE_COLOR)
            self.map.set_at(
                (self.doorways[0].pos[0]-1, self.doorways[0].pos[1]), constants.MAP_BACKGROUND_COLOR)
            self.map.set_at(
                (self.doorways[0].pos[0], self.doorways[0].pos[1]-1), constants.MAP_BACKGROUND_COLOR)
            self.map.set_at(
                (self.doorways[0].pos[0], self.doorways[0].pos[1]+1), constants.MAP_BACKGROUND_COLOR)
            self.map.set_at(
                (self.doorways[0].pos[0]+1, self.doorways[0].pos[1]), constants.PATH_TILE_COLOR)
                    ...
                    if self.map.get_at((i, j)) != constants.MAP_BACKGROUND_COLOR and self.map.get_at((i, j)) != constants.SECRET_ROOM_TILE_COLOR:
                        if self.map.get_at((i-1, j)) == constants.PATH_TILE_COLOR and self.map.get_at((i+1, j)) == constants.PATH_TILE_COLOR and self.map.get_at((i, j-1)) == constants.PATH_TILE_COLOR and self.map.get_at((i, j+1)) == constants.MAP_BACKGROUND_COLOR:
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("t_bottom", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("t_top", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("t_right", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("t_left", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("straight_y", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("straight_x", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("L_top_left", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("L_top_right", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("L_bottom_left", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("L_bottom_right", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("end_up", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("end_right", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("end_down", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("end_left", (i, j))})
                            ...
                            self.levelarray.update(
                                {str((i, j)): UnloadedLevel("cross", (i, j))})
...
class Run():
    def __init__(self, difficulty, seed=0x00000000):  # this object will be saved
        ...
        common.player.inventory["main_0"] = items.registry["gun"]
        common.player.inventory["main_1"] = items.registry["gun2"]
