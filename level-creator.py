...
class UIElement():
    def __init__(self, name, path, pos):
        ...
        )*screen_scale/2, self.image.get_height()*screen_scale/2))
        self.rect = pygame.Rect(
            pos[0]*screen_scale, pos[1]*screen_scale, self.image.get_width(), self.image.get_height())
        self.name = name
        self.isopen = False
...
for i in os.listdir("assets/level-creator"):
    ...
    if j.is_file():
        if j.suffix == ".png":
            ui_assets.update(
                {j.stem.replace("-", "_"): UIElement(j.stem.replace("-", "_"), j, (8, 8))})
...
def draw():
    ...
    pygame.draw.rect(hud, (70, 70, 70), pygame.Rect(
    ...
    if layer == 1:
        ...
        disp_win.blit(pygame.transform.scale(display, (display.get_width(
